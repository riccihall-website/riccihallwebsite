...
foreach($nameArray as $name){				// construct new line
	...
	//print($i." ".$typeArray[$i][0]." ");
	if(is_array($typeArray[$i]) && eregi("boxList",$typeArray[$i][0]) ) {   // split column if box list
		$boxListArray=explode(",",$_REQUEST[$name]);
		foreach($boxListArray as $checkbox) {
			$newline_html.="<td>".$checkbox."</td>";
		}
	} else {
		$newline_html.="<td>".eregi_replace("\r\n","<br>",stripslashes($_REQUEST[$name]))."</td>";  // html columns
	}
	$i++;
}
...
$sampleArray=getRowById($dbname.".txt",1);
//print_r($sampleArray);
?>
<script>
<!--
...
for($i=0; $i<count($nameArray);$i++) {
	...
	if ($tempshow) {		
		if ($thisarray[$i+1]) 
			showRow($nameArray[$i],$typeArray[$i],$thisarray[$i+1],$sampleArray[$i+1]);
		 else 
			showRow($nameArray[$i],$typeArray[$i],$defaultArray[$i],$sampleArray[$i+1]);
	}
}
?>
</table>
