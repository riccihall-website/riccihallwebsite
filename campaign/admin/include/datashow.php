<?
function showRow($name,$type,$value,$sample){
/////////////////////////// show row ////////////////////////////////
	global $section;
	global $dir;
	global $langArray;
	...
	case "textCheck":
		if ($sample) 
			$input="<input type=text name=\"$name\" id=\"$name\" size=60 value=\"$value\"> <span class=sample>(e.g. $sample)</span>";
		else
	...
	if (ereg("&&",$typeArray[$i])){
		...
		$typeArray[$i]=$tempselect[0];
		$typeValue=$tempselect[1];
	} else {
		$typeValue=$typeArray[$i];
	}

			$thisname=$name.$typeArray[$i];
			if ($valueArray[$typeArray[$i]-1]=="Y")
				$input.="<span style=\"display: inline-block;\"><input type=\"checkbox\" id=\"$thisname\" name=\"$thisname\" value=\"Y\" checked> $typeValue</span> &nbsp;&nbsp;";
			else
				$input.="<span style=\"display: inline-block;\"><input type=\"checkbox\" id=\"$thisname\" name=\"$thisname\" value=\"Y\"> $typeValue</span> &nbsp;&nbsp;";
		}
		break;
	case "select":
	case "selectCheck":
...
if($value) {							// show image or flash if file name found in db
	if (ereg(".swf",$value)) {
		...
		$tempfilename=$dir.$value;
		$tempsize = getimagesize($tempfilename); 
		if($tempsize[0]>400) $tempsize[0]=400;
		$input.="<br><a href=\"$dir$value\" target=_blank><img src=\"$dir$value\" width=$tempsize[0] border=0></a>";
	} else {
...
if($value) {
	...
	} elseif(ereg(".jpg",$value)||ereg(".gif",$value) || eregi(".png",$value)) {
		$tempfilename=$dir.$value;
		$tempsize = getimagesize($tempfilename); 
		if($tempsize[0]>400) $tempsize[0]=400;
		$input=$input2;
...
/////////////////////////// show extra ////////////////////////////////
	...
	case "boxListCheck":
		...
		print("\tfor(i=1;i<=".count($typeArray).";i++) {\n");
		print("\t\tif (i>1) dm=',';\n");
		print("\t\tif (document.getElementById('$name'+i) && document.getElementById('$name'+i).checked)\n");
		print("\t\t\tdocument.getElementById('$name').value+=dm+'Y';\n");
		print("\t\telse\n\t\t\tdocument.getElementById('$name').value+=dm+'N';\n");
		print("\t}\n");
		//print("\talert(document.getElementById('$name').value);\n");
		break;
	}
/////////////////////////// show extra ////////////////////////////////
}
